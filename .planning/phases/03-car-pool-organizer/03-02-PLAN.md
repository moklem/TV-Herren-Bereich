...
must_haves:
  ...
  - All 6 routes guard event.type !== 'Game' with 400 response
requirements:
  - CARPOOL-01
  - CARPOOL-02
  - CARPOOL-03
...
const routes = ['carpool/register','carpool/pick-driver','carpool/assign','carpool/finalize','carpool/reopen'];
...
- 6 carpool routes registered on the router (POST register, DELETE register, PATCH pick-driver, PATCH assign, POST finalize, POST reopen)
- All 6 guard against non-Game events (400)
...
- `server/routes/eventRoutes.js` — 6 new routes appended, PushSubscription + sendNotification imported
